...
class LayerWeighting(nn.Module):
    def __init__(self,
        ...
        self.weighting = nn.Parameter(torch.ones((1, num_layers, 1, 1))) # (B, L, C, T)

    def forward(self, av_feats):
        weights = torch.nn.functional.softmax(self.weighting, dim=1)
        return torch.sum(weights*av_feats, dim=1)
...
class TemporalAlignment(nn.Module):
    def __init__(self,
        ...
        if factor > 1:
            ...
            self.resample = nn.Upsample(scale_factor=factor, mode='linear')
        elif factor == 1:
            self.conv = nn.Conv1d(in_channels, out_channels, kernel_size=1, stride=1)
            self.resample = nn.Identity()
        else:
            self.conv = nn.Conv1d(in_channels, out_channels, kernel_size=int(1/factor), stride=int(1/factor))
            self.resample = nn.Identity()
...
class NCSNpp_cond1(nn.Module):
    ...
    def __init__(self,
        ...
        resampling_factors = (4, 2, 1, 1/2, 1/4, 1/8, 1/16),
        num_res_blocks = 2,
        attn_resolutions = (16,),
        ...
        **unused_kwargs
        ...
        self.num_channels = num_channels  # x.real, x.imag, y.real, y.imag
        ...
        # Layer weighting per resolustion
        layer_weighting = []
        for i in range(num_resolutions):
            layer_weighting.append(LayerWeighting())
        self.layer_weighting = nn.ModuleList(layer_weighting)
        ...
        for i_level in range(num_resolutions):
            temporal_aligment.append(TemporalAlignment(out_channels=in_ch, factor=resampling_factors[i_level]))
            # Residual blocks for this resolution
            for i_block in range(num_res_blocks):
                out_ch = nf * ch_mult[i_level]
                if i_block == 0:
                    modules.append(ResnetBlock(in_ch=2*in_ch, out_ch=out_ch)) # modified
                else:
        ...
        self.temporal_aligment = nn.ModuleList(temporal_aligment)

        in_ch = hs_c[-1]
        modules.append(ResnetBlock(in_ch=2*in_ch, out_ch=in_ch)) # modified
        modules.append(AttnBlock(channels=in_ch))
    ...
    def forward(self, x, t, sigmas=None, av_feats=None):
        ...
        # Stack features 
        av_feats = torch.stack(av_feats) # (L, T, B, C)
        av_feats = av_feats.permute(2, 0, 3, 1) # (B, L, C, T)

        if self.embedding_type == 'fourier':
            # Gaussian Fourier features embeddings.
            temb = modules[m_idx](torch.log(sigmas))
            m_idx += 1
        else:
        ...
        for i_level in range(self.num_resolutions):
            # Conditioning part
            av_feats_layer = self.layer_weighting[i_level](av_feats) # (B, C, T)
            av_feats_layer = self.temporal_aligment[i_level](av_feats_layer) # (B, C, T)
            av_feats_layer = av_feats_layer.unsqueeze(2) # (B, C, 1, T)
            av_feats_layer = av_feats_layer.expand(-1, -1, hs[-1].shape[2], -1) # (B, C, F, T)

            # Residual blocks for this resolution
            for i_block in range(self.num_res_blocks):
                if i_block == 0:
                    h = modules[m_idx](torch.cat((hs[-1], av_feats_layer), dim=1), temb)
                else:
        ...
        h = modules[m_idx](torch.cat((h, av_feats_layer), dim=1), temb)  # ResNet block
        m_idx += 1
        ...
        if self.scale_by_sigma:
            sigmas = sigmas.reshape((x.shape[0], *([1] * len(x.shape[1:]))))
            h = h / sigmas
