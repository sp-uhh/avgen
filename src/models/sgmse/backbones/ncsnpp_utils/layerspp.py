...
class Upsample(nn.Module):
  def __init__(self, in_ch=None, out_ch=None, with_conv=False, fir=False, fir1d=False,
               fir_kernel=(1, 3, 3, 1)):
    super().__init__()
  ...
  def forward(self, x):
    ...
    if not self.fir:
      ...
      if not self.with_conv:
        if self.fir1d:
          h = up_or_down_sampling.upsample_1d(x, self.fir_kernel, factor=2)
        else:
...
class ResnetBlockBigGANpp(nn.Module):
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, up=False, down=False,
               dropout=0.1, fir=False, fir1d=False, fir_kernel=(1, 3, 3, 1),
